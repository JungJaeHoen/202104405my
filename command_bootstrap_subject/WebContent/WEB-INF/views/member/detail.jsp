	                ...
	                <div class="form-group row">
	                 ...
	                 <div class="form-group row">
	                  ...
	                  <div class="col-sm-9">   
	                  	...
	                  	<input name="phone" type="text" class="form-control" id="inputPassword3" value="${phone.substring(0,3) }-${phone.substring(3,7) }-${phone.substring(7) }">	                
	                  </div>                  
	                </div>               
	              </div>  
...
<%-- 			          	<c:if test="${loginUser.id eq member.id }"> --%>
			          		...
			          		<div class="col-sm-3 text-center">
			          			<c:if test="${member.enabled ne 0 }">
			          				<button type="button" onclick="location.href='stop.do?id=${member.id}';" id="stopBtn" class="btn btn-info" >비활성</button>
			          			</c:if>
			          			<c:if test="${member.enabled ne 1 }">
			          				<button type="button" onclick="location.href='active.do?id=${member.id}';" id="stopBtn" class="btn btn-info" >활성</button>
			          			</c:if>
			          		</div>
<%-- 			           </c:if>	 --%>
