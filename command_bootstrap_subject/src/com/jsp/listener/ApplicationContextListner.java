...
public class ApplicationContextListner implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent ctxEvent)  { 
    	...
    	try {
			...
			for(int i = 0; i < beans.getLength(); i++) { //bean Instance생성
				...
				if(bean.getNodeType() == Node.ELEMENT_NODE) {
					...
					Class<?> cls = Class.forName(classType);//여기서 ? 는 와일드 카드라고한다.
					Object targetObj = cls.newInstance(); //singletone
			...
			for (int i = 0; i < beans.getLength(); i++) {
				...
				if(bean.getNodeType() == Node.ELEMENT_NODE) {
					...
					for (int j = 0; j < properties.getLength(); j++) {
						...
						if(!property.getNodeName().equals("property")) continue; //프로퍼티가 아니면 컨티뉴해서 다음
